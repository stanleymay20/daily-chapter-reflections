...
import { safeShareText } from "../app-state";
...
describe("looping schedule",()=>{
  ...
  it("matches boundary samples",()=>{
    expect(ids("2026-02-20")[0]).toBe("EXO.1");
    expect(ids("2026-05-31")[2]).toBe("PSA.1");
    expect(ids("2026-12-31")).toEqual(["DEU.25","1SA.25","PSA.65","PRO.8","JER.49","MAT.9","ACT.23"]);
  });
...
describe("sharing safety",()=>{
  it("limits licensed excerpts",()=>{const out=safeShareText("Genesis 42:1","x".repeat(400),180);expect(out.length).toBeLessThanOrEqual(200);expect(out).toContain("Genesis 42:1");expect(out.endsWith("…")).toBe(true);});
});
