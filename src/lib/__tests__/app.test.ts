...
import { isOptionalSchemaMissing } from "../cloud-sync";
...
describe("cloud schema fallback",()=>{
  it("recognizes only schema-not-ready errors as optional",()=>{expect(isOptionalSchemaMissing({code:"42P01",message:"relation does not exist"})).toBe(true);expect(isOptionalSchemaMissing({code:"PGRST205",message:"Could not find the table"})).toBe(true);expect(isOptionalSchemaMissing({code:"42501",message:"permission denied"})).toBe(false);expect(isOptionalSchemaMissing({code:"23505",message:"duplicate key"})).toBe(false)});
});
