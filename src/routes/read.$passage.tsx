...
const stages:{id:Stage;label:string;icon:typeof BookOpenCheck}[]=[
  {id:"read",label:"Read",icon:BookOpenCheck},
  {id:"understand",label:"Understand",icon:Lightbulb},
  {id:"explore",label:"Explore",icon:Workflow},
...
function Reader(){
  ...
  return <main className={`mx-auto min-h-screen px-4 pb-32 pt-5 sm:px-6 ${readingWidthClass(settings.readingWidth)}`}>
    ...
    {settings.studyMode!=="read"?<section className="mt-5 rounded-2xl border bg-card p-4"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Before you read</p><p className="mt-2 text-sm">What are you bringing into this chapter today?</p><Textarea className="mt-3 min-h-20" value={study.intention??""} onChange={e=>updateStudy({intention:e.target.value})} placeholder="A concern, gratitude, question, or simply: I am here to listen."/></section>:null}
...
function VisualSequence({title,icon:Icon,items}:{title:string;icon:typeof Workflow;items:string[]}){if(!items?.length)return null;return <Card className="p-4"><div className="flex items-center gap-2"><Icon className="size-4 text-primary"/><h3 className="text-sm font-semibold">{title}</h3></div><ol className="mt-4 space-y-0">{items.map((x,i)=><li key={i} className="relative flex gap-3 pb-4 last:pb-0"><div className="relative z-10 grid size-7 shrink-0 place-items-center rounded-full bg-primary text-xs font-semibold text-primary-foreground">{i+1}</div>{i<items.length-1?<span className="absolute left-[13px] top-7 h-[calc(100%-12px)] w-px bg-border"/>:null}<p className="pt-1 text-sm leading-relaxed">{x}</p></li>)}</ol></Card>}
function ExploreList({title,icon:Icon,items}:{title:string;icon:typeof UsersRound;items:string[]}){if(!items?.length)return null;return <Card className="p-4"><div className="flex items-center gap-2"><Icon className="size-4 text-primary"/><h3 className="text-sm font-semibold">{title}</h3></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{items.map((x,i)=><div key={i} className="rounded-xl bg-muted/60 p-3 text-sm leading-relaxed">{x}</div>)}</div></Card>}
